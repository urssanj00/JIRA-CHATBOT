...
class LocalModelManager:
    ...
    def __init__(self):
        ...
        print(f'Model directory: {self.model_dir}   model name: {self.model_name}')
...
Question: {prompt} [/INST]"""
    ...
    def enhanced_semantic_search(self, query: str, top_k: int = None) -> List[Dict]:
        """Enhanced search with optimized parameters"""
        if top_k is None:
            params = self.optimize_search_parameters()
    ...
    def handle_fast_queries(self, query: str) -> str:
        ...
        if 'critical bug' in query_lower or 'show me critical bugs' in query_lower:
            filtered = [
                ...
                if item['type'].lower() == 'bug' and item['priority'].lower() == 'critical'
            ]
            if not filtered:
    ...
    def create_training_pairs(self) -> List[Tuple[str, str]]:
        """Create question-answer pairs for fine-tuning from Jira data"""
        training_pairs = []
        ...
        for item in self.data:
            ...
            questions = [
                f"What is {issue_id}?",
                f"Tell me about {issue_id}",
                f"Show me details of {issue_id}",
                f"What is the status of {issue_id}?",
                f"What type of issue is {issue_id}?",
                f"What is the priority of {issue_id}?",
            ]
            
            # Generate type-based queries
            if 'bug' in issue_type.lower():
                questions.extend([
                    f"Show me bug {issue_id}",
                    f"What bug has summary {summary[:30]}?",
                    "Show me critical bugs" if priority.lower() == 'critical' else None,
                    "Show me high priority bugs" if priority.lower() in ['high', 'critical'] else None,
                ])
            
            # Generate status-based queries
            if status.lower() in ['open', 'in progress', 'todo']:
                questions.append(f"What issues are {status.lower()}?")
            
            # Filter None values
            questions = [q for q in questions if q is not None]
            
            # Create answer
            answer = f"{issue_type} {issue_id}: {summary}. Status: {status}, Priority: {priority}."
            if description:
                answer += f" Description: {description[:100]}..."
    ...
    def fine_tune_embeddings(self, progress_callback=None) -> bool:
        ...
        try:
            ...
            if not self.embedding_model or not self.data:
                return False
            
            if progress_callback:
                progress_callback("Creating training pairs...")
            ...
            import random
            negative_examples = []
            for i, (question, _) in enumerate(training_pairs[:100]):  # Limit for performance
                # Get random wrong answer
                wrong_answer = random.choice([pair[1] for j, pair in enumerate(training_pairs) if j != i])
                negative_examples.append(InputExample(texts=[question, wrong_answer], label=0.0))
            
            all_examples = train_examples + negative_examples
            ...
            # Fine-tune
            if progress_callback:
                progress_callback("Fine-tuning embeddings...")
            ...
            if progress_callback:
                progress_callback("Rebuilding embeddings...")
            
            self.embeddings = self.build_embeddings(self.data)
            if self.embeddings is not None:
                self.index = faiss.IndexFlatIP(self.embeddings.shape[1])
                self.index.add(self.embeddings.astype('float32'))
            ...
            print("PyTorch not available for fine-tuning")
            return False
        except Exception as e:
            print(f"Fine-tuning failed: {e}")
            return False

    def create_synthetic_qa_pairs(self) -> List[Tuple[str, str]]:
        """Create synthetic Q&A pairs for better training coverage"""
        synthetic_pairs = []
        
        # Statistical queries
        synthetic_pairs.extend([
            ("How many total issues are there?", f"There are {len(self.data)} total issues."),
            ("How many bugs are there?", f"There are {sum(1 for item in self.data if 'bug' in item['type'].lower())} bugs."),
            ("What are the most common issue types?", self._get_type_distribution()),
            ("Show me high priority issues", self._get_high_priority_summary()),
            ("What issues are open?", self._get_open_issues_summary()),
        ])
        
        # Priority-based queries
        priorities = set(item['priority'] for item in self.data)
        for priority in priorities:
            count = sum(1 for item in self.data if item['priority'] == priority)
            synthetic_pairs.append((
                f"How many {priority.lower()} priority issues?",
                f"There are {count} {priority.lower()} priority issues."
            ))
        
        # Status-based queries
        statuses = set(item['status'] for item in self.data)
        for status in statuses:
            count = sum(1 for item in self.data if item['status'] == status)
            synthetic_pairs.append((
                f"How many issues are {status.lower()}?",
                f"There are {count} issues with status {status}."
            ))
        
        return synthetic_pairs

    def _get_type_distribution(self) -> str:
        """Helper method for type distribution"""
        types = Counter(item['type'] for item in self.data)
        result = "Issue type distribution:\n"
        for issue_type, count in types.most_common(5):
            result += f"• {issue_type}: {count}\n"
        return result.strip()

    def _get_high_priority_summary(self) -> str:
        """Helper method for high priority issues"""
        high_priority = [item for item in self.data if item['priority'].lower() in ['high', 'critical']]
        if not high_priority:
            return "No high priority issues found."
        
        result = f"Found {len(high_priority)} high priority issues:\n"
        for item in high_priority[:3]:
            result += f"• {item['issue_id']}: {item['summary'][:50]}...\n"
        return result.strip()

    def _get_open_issues_summary(self) -> str:
        """Helper method for open issues"""
        open_issues = [item for item in self.data if item['status'].lower() in ['open', 'to do', 'in progress']]
        if not open_issues:
            return "No open issues found."
        
        result = f"Found {len(open_issues)} open issues:\n"
...
def create_interface():
    ...
    def train_from_json(json_path, progress=gr.Progress()):
        ...
        if success:
            # Auto fine-tune after loading
            if chatbot.fine_tune_embeddings(progress_callback):
                status_msg = f"Training + Fine-tuning completed! {len(chatbot.data)} issues loaded."
            else:
                status_msg = f"Training completed! {len(chatbot.data)} issues loaded. (Fine-tuning skipped)"
    ...
    with gr.Blocks(title="Llama 2 7B Jira Chatbot", theme=gr.themes.Soft()) as demo:
        gr.Markdown("# 🦙 Llama 2 7B Jira Chatbot - JSON Training")
