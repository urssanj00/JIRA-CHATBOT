...
Streamlined version with JSON data processing and pickle conversion
"""
import os
import json
import pickle
...
json_path ="/Users/sanjeev/VNIT/JIRA-CHATBOT/models"
...
class FineTunedJiraChatbot:
    ...
    def json_to_pickle(self, json_path: str, progress_callback=None) -> str:
        ...
        try:
            ...
            #print(f'items from JSON: {json_data}   ')
            ...
            if isinstance(item, dict):
              #  print(303)
               ...
               # print(304)
                ...
                input_text = f"""Issue ID: {issue_id}, Type: {issue_type}
Summary: {summary}
Status: {status}
Priority: {priority}
...
Question: {prompt} [/INST]"""
    ...
    def semantic_search_enhanced(self, query: str, top_k: int = 5) -> List[Dict]:
        ...
        try:
            ...
            for i, idx in enumerate(indices[0]):
                if idx < len(self.data) and scores[0][i] > 0.3:
    ...
    def handle_fast_queries(self, query: str) -> str:
        """Handle fast statistical queries"""
        if not self.data:
...
def create_interface():
    ...
    print(f'b json_path {json_path}')

    def train_from_json(json_path, progress=gr.Progress()):
        print(f'c json_path:{json_path}')
        ...
        if success:
            ...
            print('h')
            return f"Training completed! {len(chatbot.data)} issues loaded.", "Ready ✓", pickle_info
        else:
    ...
    def train_from_pickle(pickle_path, progress=gr.Progress()):
        ...
        print(f'i pickle_path: {pickle_path}')
        def progress_callback(msg):
            progress(0.5, desc=msg)
    ...
    with gr.Blocks(title="Llama 2 7B Jira Chatbot", theme=gr.themes.Soft()) as demo:
        ...
        train_json_btn.click(train_from_json, inputs=[json_file], outputs=[train_status, model_status, pickle_path_display])
        print(38)
        # train_pickle_btn.click(train_from_pickle, inputs=[pickle_file], outputs=[train_status, model_status])
        train_pickle_btn.click(train_from_pickle, inputs=[pickle_file], outputs=[train_status, model_status, pickle_path_display])
...
#    demo.launch(server_name="0.0.0.0", server_port=7860,share=True)
    ...
    #demo.launch(share=False, debug=True, server_name="192.168.1.20", server_port=7860)